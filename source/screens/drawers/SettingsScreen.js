  ...
  try {
    ...
    if (isRevoke) updatePayload = { login_devices: [] };
    if (isReactivate) updatePayload = { is_deleted: false };
    if (isDeactivate) updatePayload = { is_deleted: true };

    // ======================================================
    // ✅ Update DB safely (only changed column)
    // ======================================================
    await dispatch(updateUser(freshUser.id, updatePayload));
...

{
  selUser?.is_admin &&  
              ...
              <TouchableOpacity
                ...
                onPress={() => navigation.navigate('UsersTree')}>
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start' }}>
                  <Image
