...
const SettingsScreen = ({ navigation }) => {
  ...
  const loadUsers = async () => {
    let localUsers = await getLocalUsers(); // SQLite cache

    if (!localUsers || localUsers.length === 0) {
      // Fetch from Supabase if local empty
      const { data: supaUsers, error } = await supabase.from('users').select('*');
      if (!error && supaUsers) {
        localUsers = supaUsers;
        await saveLocalUsers(localUsers); // Cache locally
      }
    }
    ...
    const channel = supabase
      ...
      .on(
        ...
        payload => {
          ...
          saveLocalUsers([updatedUser]); // Update SQLite cache
        }
      )
      .subscribe();
...
const signOut = useCallback(async () => {
  ...
  const ableToViewDeletedUsers = getConfiguration(selUser, 'deletedUsers')?.is_check;
  const ableToViewAppUsers = getConfiguration(selUser, 'appUsers')?.is_check;
  const ableToViewMap = getConfiguration(selUser, 'mapUsers')?.is_check;
